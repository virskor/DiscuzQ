...
class DiscuzAppBar extends StatefulWidget implements PreferredSizeWidget {
  ...
  DiscuzAppBar({
    ...
    this.textTheme,
    this.primary = true,
    this.shadowColor = const Color(0x1F555555),
...
class _DiscuzAppBarState extends State<DiscuzAppBar> {
  ...
  Widget build(BuildContext context) {
    return Consumer<AppConfigProvider>(
      ...
      final Color buildBackgroundColor = conf.appConf['darkTheme']
...
class DiscuzSliverAppBar extends StatefulWidget {
  /// Creates a material design app bar that can be placed in a [CustomScrollView].
  ///
  /// The arguments [forceElevated], [primary], [floating], [pinned], [snap]
  /// and [automaticallyImplyLeading] must not be null.
  const DiscuzSliverAppBar({
    Key key,
    this.leading,
    this.automaticallyImplyLeading = true,
    this.title,
  ...
  /// be the same as the app bar's overall height.
  ///
  /// When using [DiscuzSliverAppBar.flexibleSpace], the [DiscuzSliverAppBar.expandedHeight]
  /// must be large enough to accommodate the [DiscuzSliverAppBar.flexibleSpace] widget.
  ///
  /// Typically a [FlexibleSpaceBar]. See [FlexibleSpaceBar] for details.
  final Widget flexibleSpace;
  ...
  /// See also:
  ///
  ///  * [DiscuzSliverAppBar] for more animated examples of how this property changes the
  ///    behavior of the app bar in combination with [pinned] and [snap].
  final bool floating;
  ...
  @override
  _DiscuzSliverAppBarState createState() => _DiscuzSliverAppBarState();
}

// This class is only Stateful because it owns the TickerProvider used
// by the floating appbar snap animation (via FloatingHeaderSnapConfiguration).
class _DiscuzSliverAppBarState extends State<DiscuzSliverAppBar> with TickerProviderStateMixin {
  FloatingHeaderSnapConfiguration _snapConfiguration;
  OverScrollHeaderStretchConfiguration _stretchConfiguration;
  ...
  @override
  void didUpdateWidget(DiscuzSliverAppBar oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (widget.snap != oldWidget.snap || widget.floating != oldWidget.floating)
      _updateSnapConfiguration();
  ...
  Widget build(BuildContext context) {
    ...
    return MediaQuery.removePadding(
      ...
      child: SliverPersistentHeader(
        floating: widget.floating,
        pinned: widget.pinned,
        delegate: _DiscuzSliverAppBarDelegate(
          vsync: this,
          leading: widget.leading,
          automaticallyImplyLeading: widget.automaticallyImplyLeading,
          title: widget.title,
...
class _DiscuzSliverAppBarDelegate extends SliverPersistentHeaderDelegate {
  _DiscuzSliverAppBarDelegate({
    @required this.leading,
    @required this.automaticallyImplyLeading,
    @required this.title,
    @required this.actions,
  ...
  @override
  bool shouldRebuild(covariant _DiscuzSliverAppBarDelegate oldDelegate) {
    return leading != oldDelegate.leading
        || automaticallyImplyLeading != oldDelegate.automaticallyImplyLeading
        || title != oldDelegate.title
        || actions != oldDelegate.actions
...
// A wrapper for the widget created by _DiscuzSliverAppBarDelegate that starts and
// stops the floating app bar's snap-into-view or snap-out-of-view animation.
