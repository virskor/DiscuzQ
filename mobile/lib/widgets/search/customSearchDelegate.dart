...
/// [DiscuzCustomSearchDelegate.close] call. If the search page is closed with the system
/// back button, it returns null.
///
/// A given [DiscuzCustomSearchDelegate] can only be associated with one active [showSearch]
/// call. Call [DiscuzCustomSearchDelegate.close] before re-using the same delegate instance
...
/// The animation provided by [DiscuzCustomSearchDelegate.transitionAnimation] can be used
...
/// See also:
///
///  * [DiscuzCustomSearchDelegate] to define the content of the search page.
Future<T> showDiscuzSearch<T>({
  @required BuildContext context,
  @required DiscuzCustomSearchDelegate<T> delegate,
  String query = '',
}) {
...
/// query text field can be customized via [DiscuzCustomSearchDelegate.buildLeading] and
/// [DiscuzCustomSearchDelegate.buildActions].
///
/// The body below the [AppBar] can either show suggested queries (returned by
/// [DiscuzCustomSearchDelegate.buildSuggestions]) or - once the user submits a search  - the
/// results of the search as returned by [DiscuzCustomSearchDelegate.buildResults].
///
/// [DiscuzCustomSearchDelegate.query] always contains the current query entered by the user
/// and should be used to build the suggestions and results.
///
/// The results can be brought on screen by calling [DiscuzCustomSearchDelegate.showResults]
/// and you can go back to showing the suggestions by calling
/// [DiscuzCustomSearchDelegate.showSuggestions].
///
/// Once the user has selected a search result, [DiscuzCustomSearchDelegate.close] should be
...
/// call. Call [DiscuzCustomSearchDelegate.close] before re-using the same delegate instance
/// for another [showSearch] call.
abstract class DiscuzCustomSearchDelegate<T> {
  /// Constructor to be called by subclasses which may specify [searchFieldLabel], [keyboardType] and/or
  /// [textInputAction].
  ///
  /// {@tool snippet}
  /// ```dart
  /// class CustomSearchHintDelegate extends DiscuzCustomSearchDelegate {
...
enum _SearchBody {
  ...
  ///
  /// The suggested queries are generated by [DiscuzCustomSearchDelegate.buildSuggestions].
  suggestions,

  /// Search results are currently shown in the body.
  ///
  /// The search results are generated by [DiscuzCustomSearchDelegate.buildResults].
  results,
}
...
class _SearchPageRoute<T> extends PageRoute<T> {
  _SearchPageRoute({
    ...
    assert(
      ...
      'search. Please close that search by calling close() on the DiscuzCustomSearchDelegate '
      'before opening another search with the same delegate instance.',
    );
    delegate._route = this;
  }

  final DiscuzCustomSearchDelegate<T> delegate;
