...
class AppConfigurations {
	...
	final ClodraLocalStorage _localStorage = ClodraLocalStorage();
	...
	Future<bool> update(
		...
		if (reverse == true) {
			await _localStorage.setString(_appConfKey, jsonEncode(defaultAppSetting));
	...
	Future<bool> _modifyLocalSetting({@required dynamic appConf}) async {
		return _localStorage.setString(_appConfKey, jsonEncode(appConf));
	}

	/// return dynamic data
	Future<dynamic> getLocalAppSetting(
		{bool returnDefaultValueIfNotExits = false}) async {
		final String appconf = await _localStorage.getString(_appConfKey);
